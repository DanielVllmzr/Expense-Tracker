...
addBtn.onclick=async ()=>{
  try{
    if(!window.dataStore || !window.dataStore.addExpense){
      alert('Inicializando base... intentá en 1-2 segundos.'); return;
    }
    ...
    await window.dataStore.addExpense({ amount:Math.abs(amount), category:cat, type:addType, createdAtISO:at.toISOString(), note });
    amountEl.value=''; if(noteEl) noteEl.value='';
  }catch(err){
    console.error('[Agregar] Error',err); alert('No se pudo guardar: '+(err?.message||err));
...
function render(){
  ...
  const filtered=records.filter(r=>{
    if(meta.viewMode==='day' && localYMDfromISO(r.createdAt)!==meta.selectedDate) return false;
    if(meta.viewMode==='month' && localYMDfromISO(r.createdAt).slice(0,7)!==meta.selectedDate.slice(0,7)) return false;
  ...
  filtered.forEach(r=>{
    ...
    const date=new Date(r.createdAt).toLocaleString(); const noteTxt = r.note ? `<div class=\"meta\">${r.note}</div>` : '';
    const sign=r.type==='income'?'+':'-';
    row.innerHTML=`
      <div class="left">
        <span class="dot" style="background:${c.color}"></span>
        <div><div>${c.name}</div><div class=\"meta\">${date}</div>${noteTxt}</div>
      </div>
      <div>
...
todayBtn.onclick=()=>{ meta.viewMode='day'; meta.selectedDate=todayYMD(); datePicker.value=meta.selectedDate; saveMeta(); render(); };
