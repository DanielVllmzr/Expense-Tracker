...
function allCats(){
  ...
  const i=arr.findIndex(c=>c.key==='todas');
  if(i>0){ const [a]=arr.splice(i,1); arr.unshift(a); }
  return arr;
}
...
if (manageCatsBtn){
  manageCatsBtn.onclick=()=>{
    const action = prompt('Categorías personalizadas\nAcción: add / rename / remove / list','add');
    const custom = loadCustomCats();
    if (action==='list'){ alert(custom.map(c=>`${c.key} = ${c.name} (${c.color})`).join('\n') || 'No tenés categorías personalizadas'); return; }
    if (action==='add'){
      const key = (prompt('Clave (ej. cafe)','')||'').trim();
      if(!key || key==='todas'){ alert('Clave inválida'); return; }
      const name= (prompt('Nombre visible', key)||'').trim();
      const color=(prompt('Color (hex, ej. #ff9800)', '#ff9800')||'').trim();
      const next = custom.filter(c=>c.key!==key); next.push({key,name,color});
      saveCustomCats(next);
    } else if (action==='rename'){
      const key = (prompt('Clave a renombrar','')||'').trim();
      const idx = custom.findIndex(c=>c.key===key);
      if(idx<0){ alert('No existe esa clave'); return; }
      const name= (prompt('Nuevo nombre', custom[idx].name)||'').trim();
      const color=(prompt('Nuevo color (hex)', custom[idx].color)||'').trim();
      custom[idx]={...custom[idx], name, color}; saveCustomCats(custom);
    } else if (action==='remove'){
      const key = (prompt('Clave a eliminar','')||'').trim();
      const next = custom.filter(c=>c.key!==key); saveCustomCats(next);
    }
...
// --- Panel visual de categorías personalizadas ---
...
function renderCatsPanel(){
  ...
  custom.forEach(c=>{
    ...
    row.className='cats-item';
    row.innerHTML = `
      <input class="control key" value="${c.key}" disabled>
      <input class="control name" value="${c.name}">
      <input class="control color" type="color" value="${c.color||'#888888'}">
      <div class="row-actions">
        <button class="ghost saveBtn" type="button">Guardar</button>
        <button class="danger delBtn" type="button">Eliminar</button>
      </div>`;
    const save = row.querySelector('.saveBtn');
    const del  = row.querySelector('.delBtn');
    save.onclick = ()=>{
      const name = row.querySelector('.name').value.trim()||c.key;
      const color = row.querySelector('.color').value.trim()||'#888888';
      const next = custom.map(x=> x.key===c.key ? {...x, name, color} : x);
      saveCustomCats(next);
      populateCategories(); drawChips(); render();
    };
    del.onclick = ()=>{
      const next = custom.filter(x=>x.key!==c.key);
      saveCustomCats(next);
      renderCatsPanel();
      populateCategories(); drawChips(); render();
    };
...
if (manageCatsBtn){
  manageCatsBtn.onclick=()=>{
    if(!catsPanel) return;
    const show = catsPanel.style.display==='none';
    catsPanel.style.display = show ? 'block' : 'none';
    if(show) renderCatsPanel();
  };
}
if (addCatBtn){
  addCatBtn.onclick=()=>{
    const key=(catKey.value||'').trim();
    const name=(catName.value||'').trim()||key;
    const color=(catColor.value||'').trim()||'#ff9800';
    if(!key || /\s/.test(key) || key==='todas'){ alert('Clave inválida'); return; }
    const custom = loadCustomCats();
    if(custom.some(c=>c.key===key)){ alert('Ya existe esa clave'); return; }
    custom.push({key,name,color});
